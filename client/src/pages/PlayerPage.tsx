...
function Vinyl({ playing }: { playing: boolean }) {
  return (
    <div className="relative w-[320px] h-[320px] animate-vinyl-float">
      <div className={`w-[320px] h-[320px] rounded-full bg-[conic-gradient(from_0deg,#1a1a2e,#16162a,#1a1a2e,#0f0f1f,#1a1a2e,#16162a,#1a1a2e)] flex items-center justify-center relative border border-white/5 ${playing ? 'animate-vinyl-spin' : ''}`}>
        ...
        <div className="w-[160px] h-[160px] rounded-full bg-gradient-to-br from-[#1a0533] via-[#2d1b69] to-[#1a0533] flex items-center justify-center border-2 border-white/10 relative overflow-hidden">
          <span className="text-[42px] opacity-70">♫</span>
...
function ShareButtons({ slug }: { slug: string }) {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <button onClick={() => navigator.clipboard.writeText(`https://${url}`)} className={btn}>
...
export default function PlayerPage() {
  ...
  useEffect(() => {
    ...
    function fetchListeners() {
      api.get(`/public/stations/${slug}/listeners`)
        .then((res) => setListeners(res.data.count ?? 0))
        .catch(() => {})
    }
    fetchListeners()
  ...
  return (
    <div className="relative min-h-screen bg-dark-surface text-white grid grid-cols-2 overflow-hidden">
      ...
      <div className="absolute top-0 left-1/2 w-px h-full bg-gradient-to-b from-transparent via-violet-full/15 to-transparent z-[1]" />
      <div className="absolute top-6 right-6 text-[11px] tracking-[4px] uppercase text-text-dim z-[3]" style={{ writingMode: 'vertical-rl' }}>
        internet radio
      </div>

      <div className="relative flex items-center justify-center p-12 z-2">
        <Vinyl playing={playing} />
      </div>

      <div className="relative flex flex-col justify-center pr-12 pl-4 py-12 z-2">
        ...
        <h1 className="text-5xl font-medium -tracking-wide leading-[1.1] text-text-primary mb-3">
          {station.name}
        </h1>

        {station.description && (
          <p className="text-[15px] text-text-muted leading-relaxed mb-8 max-w-[340px]">
            {station.description}
          </p>
        )}

        {station.is_live && (
          <div className="flex items-center gap-3 mb-10 px-4 py-3 bg-white/[0.04] rounded-xl border border-border-faint">
            <div>
        ...
        <div className="flex items-center gap-5 mb-8">
          <button
            onClick={togglePlay}
            className="w-16 h-16 rounded-full bg-violet flex items-center justify-center shrink-0 border-none cursor-pointer hover:scale-[1.08] hover:bg-violet-full transition-all"
          >
            {playing ? (
      ...
      <div className="absolute bottom-0 left-0 right-0 px-12 py-4 flex justify-between items-center z-[3] border-t border-white/5">
        ...
        <div className="text-xs text-text-ghost">gocast.fm/{station.slug}</div>
      </div>
    </div>
  )
}
